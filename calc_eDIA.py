...
def calculate_region_stats(df, bootstrap_n=500):
    ...
    df.loc[:, 'hover_text_DIA'] = [f"{df.loc[i, 'region_info']}<br>eDIA: {df.loc[i, 'dia']:.2f} (95% CI: {df.loc[i, 'dia_lower']:.2f}-{df.loc[i, 'dia_upper']:.2f})" for i in range(df.shape[0])]
...
def make_plot(wg_info, region_stats, x_vals, x_text, output_file):
    fig = make_subplots(
        ...
        specs=[[{"colspan": 2}, None],
            ...
            [{"colspan": 2}, None]],
        subplot_titles=(f"Whole Genome eDIA Bootstrap Distribution<br>{wg_info['hover_text']}", 
                        "Copy Number & Allele Frequency", 
                        "Split Region eDIA"),
    ...
    fig.update_yaxes(title_text='Allele Frequency', row=2, col=1, secondary_y=False, range=[0,1])
    fig.update_yaxes(title_text="Ploidy", row=2, col=1, secondary_y=True)
    fig.update_yaxes(title_text='eDIA', row=3, col=1, type='log')
...
if __name__ == "__main__":
    ...
    if args.bootstrap > 0 and isinstance(bootstrap_eDIA, list) and len(bootstrap_eDIA) > 0:
        ...
        if args.verbosity == 'normal':
            print(f"Whole Genome eDIA: {eDIA:.2f} (95% CI: {wg_lower:.2f}-{wg_upper:.2f})\n")
        ...
        if args.verbosity == 'normal':
            print(f"Whole Genome eDIA: {eDIA:.2f}\n")
    wg_info = {
        'dia': eDIA,
        'boots': bootstrap_eDIA,
        'hover_text': f"eDIA: {eDIA:.2f} (95% CI: {wg_lower:.2f}-{wg_upper:.2f})" if args.bootstrap > 0 else f"eDIA: {eDIA:.2f}",
